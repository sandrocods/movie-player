...
<title>Movie Player | ADMIN</title>
  ...
  <div class="container">
    <h1 class="display-4">Admin Panel | SaveFilm21  </h1>
...
$Link = $_GET['link'];
$movieName = str_replace(array("/","http:","103.194.171.18"), '', $Link);
$links = "http://103.194.171.18/". $movieName. "/play#down";
$FetchId = curl(
    $links,
    'GET',
    null,
...
$id = getStr($FetchId['Body'], 'href="https://femax20.com/f/','"');
if (!empty($id)) {
    $scrap = curl(
    ...
    <p>Silahkan Pilih Resolusi Video Anda : '.$links.'</p>
  </div>';
}else{
