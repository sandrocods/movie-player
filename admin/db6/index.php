...
<?php
    if (!isset($_POST['link']) || empty($_POST['link'])) {
        ...
        if (!empty($_POST['genre'])) {
          $parm = 'http://85.114.138.56/genre/action/'.$_POST['genre'].'/page/'. $_POST['link'].'/';
        } else {
          $parm = 'http://85.114.138.56/movies/page/'. $_POST['link'].'/';
        }
        if (!is_numeric($_POST['link'])){
          $parm = 'http://85.114.138.56/?s='. $_POST['link'];
        }
        echo '
          <center>
...
$Scrap = curl(
    ...
    [
        ...
        'Referer: http://85.114.138.56/movies/',
        'Connection: keep-alive'
    ],
    null
);
preg_match_all('/<a href=\'(.*?)\' data-url=\'\'|<img src=\'(.*?)\' class=\'lazy thumb mli-thumb\'/m', $Scrap['Body'], $out);
...
<td><a href="./../db6/process_scrap.php?link='.$link[$key].'" class="btn btn-secondary" tabindex="-1" role="button" aria-disabled="false">Add to database</a></td>
</tr>
</tbody>
</table>
</div>
